...
UI_THEME            = 'goodsceptic-adaptive.sublime-theme'
BRIGHT_COLOR_SCHEME = pjoin(PLUGIN_DIR_REL, 'goodsceptic-bright.sublime-color-scheme')
DARK_COLOR_SCHEME   = pjoin(PLUGIN_DIR_REL, 'goodsceptic-dark.sublime-color-scheme')
...
def set_settings(**kwargs):
  ...
  sublime.status_message('goodsceptic-theme: set settings ' + repr(kwargs.keys()))

# note: sublime maps the name of each top level class to a command
# through the naming convention: GoodscepticToggleTheme => "goodsceptic_toggle_theme"

class GoodscepticToggleThemeCommand(sublime_plugin.WindowCommand):
  def run(self, **kwargs):
...
class GoodscepticSelectThemeCommand(sublime_plugin.WindowCommand):
  def run(self, **kwargs):
  ...
  def selectScheme(self):
    ...
    if len(color_schemes) == 0:
      print("goodsceptic: no color schemes found in", PLUGIN_DIR)
      sublime.status_message("goodsceptic: no color schemes found in %s" % PLUGIN_DIR)
      return
